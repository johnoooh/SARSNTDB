...
// genomeCanvas is checbox for showing other proteins on canvas
...
function getGenomeCanvasData(pData, toggleFullGenome) 
{
    ...
    for(var i=0;i < pData.length; i++) 
    {
        ...
        if(row['Current_Protein']) 
        {
            ...
            plotRowData(row, row['Start'], row['End'], fillColor, dataHeight, hasBlackBackground); 

        } else if (toggleFullGenome) {
            plotRowData(row, row['Start'], row['End'], fillColor, dataHeight, false);  
        }
        console.log(row)
